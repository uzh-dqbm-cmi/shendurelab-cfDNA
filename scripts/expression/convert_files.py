...
for cid in sorted(geneIDs):
  if os.path.exists("%s/%s/fft%s/%s/block_%s.tsv.gz"%(tempDir,project,options.suffix,ind,cid)):
    if not wrote_header:
      header = ['#Region']
      infile = gzip.open("%s/%s/fft%s/%s/block_%s.tsv.gz"%(tempDir,project,options.suffix,ind,cid), "rt")
      infile.readline()
      for line in infile:
        fields = line.split()
